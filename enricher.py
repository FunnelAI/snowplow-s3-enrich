""" Enricher with headers
This produces a TSV-like output which includes headers
"""
import gzip
import logging
import re
...
def enrich(bucket_name, file_key, S3=boto3.client('s3')):
    ...
    for l in lines[2:-1]:
        ...
        geoipdbreader = geoip2.database.Reader('./GeoLite2-City.mmdb')
        user_ipaddress = l[4]
        geoipdbresult = geoipdbreader.city(l[4])
        geo_country = geoipdbresult.registered_country.iso_code
        geo_city = geoipdbresult.city.names.get('en', '-')
        ...
        geo_region_name = geoipdbresult.subdivisions[0].names.get('en', '-') if any(geoipdbresult.subdivisions) else '-'
    ...
    if len(urispltnodes[0]) > 5:
        ...
        S3.put_object(Bucket=bucket_name,
                      Key=file_key.replace("RAW", "Converted"),
